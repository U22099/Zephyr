...
export function Chat() {
  ...
  return (
    <motion.main className="w-screen flex flex-col" initial={{x: 300}} animate={{x: 0}} exit={{x: 300}} transition={{duration: 0.3}}>
      <header className="sticky top-0 left-0 w-full grid grid-cols-12 gap-1 backdrop-blur-sm pb-2 px-1 border-b z-10 items-center text-center pt-1">
        <FaChevronLeft className="self-center dark:fill-white fill-black w-6 h-6 col-span-2 mx-1" onClick={() => setPage({open: false, component: 'default'})}/>
        <section className="flex items-center gap-2 col-span-6 w-full">
        ...
        <HiOutlinePhone className="self-center dark:stroke-white stroke-black border w-6 h-6 col-span-2 mx-2"/>
        <IoVideocamOutline className="self-center dark:stroke-white stroke-black border w-6 h-6 col-span-2 text-lg mx-2"/>
      </header>
