  ...
  return (
    <motion.main initial={{x: 300}} animate={{x: 0}} exit={{x: 300}} transition={{duration: 0.3}} className="flex h-full flex-col items-center justify-start w-full gap-1 p-2">
      ...
      <section className="flex flex-col items-center justify-center gap-1 break-words mb-10">
        ...
        <div className="flex justify-center gap-2 items-center flex-wrap">{page.data.status === <p className="text-md text-muted-foreground font-semibold border p-2 rounded backdrop-blur-sm"> online</p> ? page.data.status : page.data.members?.map((member, i) => <p key={i} className="text-md text-muted-foreground font-semibold border p-2 rounded backdrop-blur-sm">{member}</p>)}</div>
      </section>
