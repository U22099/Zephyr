  ...
  return(
    <main className="flex gap-2 active:bg-gray-800 w-full" onClick={() => setPage({
      ...
      <section className="py-1 h-full flex flex-col justify-center border-b gap-1 w-full">
        ...
        {doc.lastMessage.type === "text" ? <p className={(!doc.lastMessage.read&&doc.lastMessage.senderId != uid ? "text-primary font-bold " : "") + "w-full truncate text-sm text-muted-foreground"}>{(doc.type === "group")&&!(doc.lastMessage.senderId === uid) ? doc.lastMessage.senderName+": " : (doc.lastMessage.senderId === uid) ? "You: " : ""}{doc.lastMessage.content || ""}</p>
        : ["image", "audio", "video", "file"].includes(doc.lastMessage.type) ? 
        <div className={(!doc.lastMessage.read&&doc.lastMessage.senderId != uid ? "text-primary fill-primary font-bold " : "text-muted-foreground ") + "text-sm flex gap-1 items-center"}>
